sanjaytestModule.controller("crudgridController", crudgridController);

function crudgridController($scope, $element, $attrs, ajaxServiceFactory, MY_notifi_Fac, modal_MY_Fac, MY_Fac) {
    ...
    self.setOrderByColumn = _sort_OrderByColumn;
    ...
    //// PRIVATE fields
    var _itemsService;
    var _createItemThrottle = MY_Fac.getThrottle(500);
    var _updateItemThrottle = MY_Fac.getThrottle(500);
    var _filterThrottle = MY_Fac.getThrottle(500);
    ...
    function _toggleEditMode(item) {
        ...
        if (!item.editMode) {
            ...
            self.CompleteItems.forEach(function (i) {
        ...
        if (_isValid(item)) {
            _itemsService.save(item,
                ...
                function (createdItem) {
                    ...
                    self.CompleteItems.unshift(createdItem);
                    self.addMode = false;
    ...
    function _deleteItemWithConfirmation(item) {
        ...
        var DeleteFromDB = function () {
            return _itemsService.delete(
                ...
                function () {
                    ...
                    // Remove from scope
                    var index = self.CompleteItems.indexOf(item);
                    self.CompleteItems.splice(index, 1);
                },
                // error callback
                function (error) {
                    _requestError(error.data);
        ...
        var msg = "Are you sure you want to remove '" + item.name + "' from this grid?";
        modal_MY_Fac.show("", msg, DeleteFromDB);
    ...
    function _getAllItems() {
        self.loading = true;
        self.CompleteItems = _itemsService.query(
            // success callback
            function () {
                self.loading = false;
    ...
    function _sort_OrderByColumn(column) {
    ...
    function _itemCreated(item) {
        ...
        self.CompleteItems.unshift(item);
    }
    ...
    function _requestSuccess() {
        MY_notifi_Fac.success();
    };

    function _requestError(error) {
        MY_notifi_Fac.error(error);
    };
    ...
    function _applyOrder() {
        self.CompleteItems.sort(function (a, b) {
